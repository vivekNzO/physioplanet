    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[801px] max-h-[90vh] overflow-y-auto px-[99px] pt-[29px] pb-[55px] rounded-lg">
              ...
              {leftColumn.map((pkg, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className="text-[#1D5287] font-bold text-base">₹{pkg.finalPrice || pkg.price}</div>
                  </div>
                  {expandedId === pkg.id && pkg.packageItems && pkg.packageItems.length > 0 && (
                    <div className="bg-white space-y-2 pt-2">
                      {pkg.packageItems.map(item => (
                        <div
                          key={item.id}
                          className="bg-[#E3F0D9] border border-[#ABD28C] rounded-md p-2.5 text-sm"
                        >
                          <div className="flex items-center justify-between">
                            ...
                            {item.servicePrice?.price && (
                              <span className="text-xs text-[#1D5287] font-semibold">
                                ₹{item.servicePrice.price}
                              </span>
                            )}
                          </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button disabled={!selectedId} onClick={handleSelect} className="bg-[#1D5287] text-white">Select Plan</Button>
        </div>
          </>
        ) : (
          <>
